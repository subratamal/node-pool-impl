...
exports.countListResults = function (html) {
	...
	if (process.env.OPTIMIZED_HTML_PARSE) {
		const tmpHtml = html.match(/(<div id="topLink.*)<div id="topmessages">/sm)
		html = tmpHtml[1]
	}
...
exports.countListPages = function (html) {
	...
	if (process.env.OPTIMIZED_HTML_PARSE) {
		const tmpHtml = html.match(/(<div class="pager rel clr.*)<div class="clr tcenter margintop20 rel">/sm)
		html = tmpHtml[1]
	}
	...
	if (!$item.length) {
		...
		const $list = /class="rel listHandler/.test(html)
		if ($list) return 1
...
exports.adInfo = function (html) {
	...
	if (process.env.OPTIMIZED_HTML_PARSE) {
		hasPhoneNumber = html.match(/contact-button\s+link-phone/m) ? true : false
	} else {
		$doc = cheerio.load(html)
		hasPhoneNumber = $doc('.contact-button.link-phone').length > 0
	}
	...
	let textAddress
	if (process.env.OPTIMIZED_HTML_PARSE) {
		const textAddressGroups = html.match(/"show-map-link".*<strong>(.*)<\/strong>/m)
		textAddress = textAddressGroups ? textAddressGroups[1] : ''
	} else {
		textAddress = lodash.trim($doc('.show-map-link strong').text())
	}
